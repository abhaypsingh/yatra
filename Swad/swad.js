/* ============================================================
   SWAD — the taste
   A record of the flavours that carry your memories
   ============================================================ */
...
(function () {
  ...
  /* ==================== STATE ==================== */
  var STORE_KEY = 'yatra-swad-tastes';
  var items = [];
  ...
  function render() {
    ...
    items.forEach(function (p) {
      ...
      name.textContent = p.dish;
      card.appendChild(name);

      if (p.memory) {
        var text = document.createElement('p');
        text.className = 'place-text';
        text.textContent = p.memory;
  ...
  function showDetail(p) {
    detailName.textContent = p.dish;
    detailText.textContent = p.memory || '';
    var d = new Date(p.timestamp);
    detailDate.textContent = 'savoured ' + d.toLocaleDateString(undefined, {
  ...
  function saveItem() {
    var dish = writeName.value.trim();
    if (!dish) return;
    var memory = writeInput.value.trim();
    items.push({ dish: dish, memory: memory, timestamp: Date.now() });
    save();
    closeWrite();
    render();
  }
