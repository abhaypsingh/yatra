/* ============================================================
   RANG — the colour
   Name the colours that speak to you and what they say
   ============================================================ */
...
(function () {
  ...
  /* ==================== STATE ==================== */
  var STORE_KEY = 'yatra-rang-colours';
  var colours = [];

  function load() {
    try {
      var raw = localStorage.getItem(STORE_KEY);
      if (raw) colours = JSON.parse(raw);
    } catch (e) {}
  }

  function save() {
    try { localStorage.setItem(STORE_KEY, JSON.stringify(colours)); }
    catch (e) {}
  }
  ...
  function render() {
    ...
    colours.forEach(function (c) {
      ...
      name.textContent = c.colour;
      card.appendChild(name);

      if (c.meaning) {
        var text = document.createElement('p');
        text.className = 'place-text';
        text.textContent = c.meaning;
  ...
  function showDetail(c) {
    detailName.textContent = c.colour;
    detailText.textContent = c.meaning || '';
  ...
  function saveColour() {
    var colour = writeName.value.trim();
    if (!colour) return;
    var meaning = writeInput.value.trim();
    colours.push({ colour: colour, meaning: meaning, timestamp: Date.now() });
    save();
    closeWrite();
    render();
  }
  ...
  writeSave.addEventListener('click', saveColour);
  writeInput.addEventListener('keydown', function (e) {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); saveColour(); }
  });
