/* ============================================================
   VIRAH — the longing
   A record of the things you miss
   ============================================================ */
...
(function () {
  ...
  var longingsEl   = document.getElementById('longings');
  var longingsEmpty = document.getElementById('longingsEmpty');
  ...
  /* ==================== STATE ==================== */
  var STORE_KEY = 'yatra-virah-longings';
  var items = [];
  ...
  /* ==================== RENDER ==================== */
  function render() {
    longingsEl.innerHTML = '';

    if (items.length === 0) {
      longingsEmpty.classList.remove('hidden');
      return;
    }

    longingsEmpty.classList.add('hidden');

    items.forEach(function (p) {
      var card = document.createElement('div');
      card.className = 'longing-card';

      var pin = document.createElement('div');
      pin.className = 'longing-pin';
      pin.textContent = '\uD83C\uDF11';
      card.appendChild(pin);

      var name = document.createElement('p');
      name.className = 'longing-name';
      name.textContent = p.what;
      card.appendChild(name);

      if (p.feeling) {
        var text = document.createElement('p');
        text.className = 'longing-text';
        text.textContent = p.feeling;
        card.appendChild(text);
      }

      card.addEventListener('click', function () { showDetail(p); });
      longingsEl.appendChild(card);
    });
  }
  ...
  function showDetail(p) {
    ...
    detailDate.textContent = 'written ' + d.toLocaleDateString(undefined, {
